...
def load_model_and_predict(rgb_path: str, checkpoint_path: str):
    ...
    print(f"Expected channels: {generated_hsi.shape[-1]}, Actual channels: {actual_channels}")
    ...
    print(f"Generated HSI shape: {generated_hsi.shape}")  # Expected: (batch_size, height, width, 31)
    for j in range(generated_hsi.shape[0]):
        try:
